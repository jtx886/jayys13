...
<div class="container">
  <div class="hero" id="hero">
    ...
    <a class="hero-slide <?= $i === 0 ? 'active' : '' ?>" href="<?= u('detail.php?type=' . $m['type'] . '&id=' . $m['id']) ?>">
      ...
      <div class="hero-body">
        <span class="hero-tag">本周热门 · <?= $m['type'] === 'tv' ? '剧集' : '电影' ?></span>
        <h1 class="hero-title"><?= e($m['title']) ?></h1>
        <div class="hero-meta">
          <?php if ($m['score']): ?><span class="score">评分 <?= $m['score'] ?></span><span class="sep"></span><?php endif; ?>
          <span><?= e($m['year']) ?></span>
          <span class="sep"></span>
        ...
        <div class="hero-btns">
          <span class="btn btn-primary btn-lg">立即播放</span>
          <span class="btn btn-ghost btn-lg">查看详情</span>
        </div>
      </div>
    </a>
    <?php endforeach; ?>
    ...
    <div class="hero-side">
      <button class="hero-arrow hero-prev" aria-label="上一部">上一部</button>
      <button class="hero-arrow hero-next" aria-label="下一部">下一部</button>
    </div>
  </div>
</div>
<?php endif; ?>
...
<div class="container">
  ...
  <section class="section" style="animation:fadeInUp .5s <?= 100 + $ri * 80 ?>ms ease both">
    <div class="section-head">
      ...
      <a class="section-more" href="<?= u($more) ?>">查看更多 </a>
    </div>
...
<div class="overlay" id="notice-overlay" data-version="<?= (int)$notice['id'] ?>">
  <div class="modal notice-modal">
    ...
    <button class="btn btn-primary btn-block" id="notice-ok">我知道了</button>
  </div>
</div>
<?php endif; ?>
